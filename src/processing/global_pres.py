...
class Global:
    ...
    def display_ratings_distribution(self):
        ...
        # Save the figure
        if os.path.exists(f"{self.save_folder}/ratings_distribution") == False:
            os.makedirs(f"{self.save_folder}/ratings_distribution")
        fig.write_html(f'{self.save_folder}/ratings_distribution/raw_ratings_distribution.html')
    ...
    def plot_correlation_matrix(self, df, save_folder, cmap=['pink', 'red']):
        corr = df.corr()
    ...
    def get_spatial_data_ratings_evolving(self, df_ratings_no_text):
        ...
        # Return everything
        return df_states_no_US, df_states_US

    def plot_median_mean_evolution_time_ratings(self,df_ratings_no_text, saving_folder):
        ...
        year_list = sorted(df_ratings_no_text['year'].unique())
        ...
        fig.write_html(saving_folder + 'mean_median_over_time.html')
